...
function otsuLocalThreshold() {
    ...
    let blockSize = 15;
    let threshold = otsuThresholdBlock(gray, width, height, blockSize);
...
function otsuThresholdBlock(gray, width, height, blockSize) {
    let threshold = 0;
    let maxVariance = 0;

    // Для каждого блока вычисляем порог с использованием метода Оцу
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            let block = [];
            for (let ky = -Math.floor(blockSize / 2); ky <= Math.floor(blockSize / 2); ky++) {
                for (let kx = -Math.floor(blockSize / 2); kx <= Math.floor(blockSize / 2); kx++) {
                    ...
                    if (posX >= 0 && posX < width && posY >= 0 && posY < height) {
                        block.push(gray[posY * width + posX]);
                    }
                }
            }
            let localThreshold = otsuBlockThreshold(block);
            if (localThreshold > threshold) threshold = localThreshold;
        }
    }
...
function otsuBlockThreshold(block) {
...
function localMeanThreshold() {
    ...
    let threshold = meanThresholdBlock(gray, width, height, blockSize);
...
function meanThresholdBlock(gray, width, height, blockSize) {
    let threshold = 0;

    // Для каждого блока вычисляем порог с использованием среднего
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            let block = [];
            for (let ky = -Math.floor(blockSize / 2); ky <= Math.floor(blockSize / 2); ky++) {
            ...
            let localThreshold = block.reduce((a, b) => a + b, 0) / block.length;
            if (localThreshold > threshold) threshold = localThreshold;
        }
    }
